<div class="space-y-6">
    ...
    <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 md:p-6 bg-white dark:bg-neutral-900 shadow-sm font-sans">
        ...
        {{-- MENSAJE DE ÉXITO --}}
        @if(session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)" class="mb-4 p-4 rounded-xl bg-green-50 dark:bg-neutral-800 border border-green-200 dark:border-green-900/50 text-green-700 dark:text-green-400 text-sm font-bold flex items-center gap-2 transition-all">
                ✅ {{ session('success') }}
            </div>
        @endif
        ...
        @if($clientes->isEmpty())
            <div class="text-center py-10">
                <p class="text-neutral-500 italic">No hay clientes archivados.</p>
            </div>
        @else
                            ...
                            <tr wire:key="archived-{{ $cliente->id }}" class="hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition">
                                ...
                                <td class="p-3 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('clientes.show', $cliente->id) }}" class="p-2 rounded-lg text-neutral-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition cursor-pointer" title="Ver">👁️</a>
                                        <button wire:click="desarchivar({{ $cliente->id }})" wire:confirm="¿Restaurar cliente?" class="p-2 text-neutral-500 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 transition rounded-lg cursor-pointer" title="Restaurar">🔄</button>
                                        <button wire:click="delete({{ $cliente->id }})" wire:confirm="¿Eliminar para siempre?" class="p-2 text-neutral-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 transition rounded-lg cursor-pointer" title="Eliminar">🗑️</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
