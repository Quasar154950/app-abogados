...
Route::get('/crear-centrolegal-temporal', function () {
    $estudio = \App\Models\Estudio::firstOrCreate(
        ['slug' => 'centrolegal'],
        [
            'nombre' => 'Estudio Jurídico del Centro',
            'activo' => true,
            'plan' => 'Básico',
            'precio_suscripcion' => 30000,
            'fecha_vencimiento' => '2026-10-08',
        ]
    );

    $luciano = \App\Models\User::firstOrCreate(
        ['email' => 'luciano@centrolegal.com'],
        [
    ...
    $valentina = \App\Models\User::firstOrCreate(
        ['email' => 'valentina@centrolegal.com'],
        [
            'name' => 'Valentina Ríos',
            'password' => bcrypt('Demo1234!'),
            'role' => 'abogado',
    ...
    $ricardo = \App\Models\Cliente::firstOrCreate(
        ['email' => 'ricardo.salvatierra@example.com'],
        [
            'nombre' => 'Ricardo Salvatierra',
            'telefono' => '2494001001',
            'direccion' => 'Av. España 850, Tandil',
            'abogado_id' => $luciano->id,
        ]
    );

    $carolina = \App\Models\Cliente::firstOrCreate(
        ['email' => 'carolina.mendez@example.com'],
        [
            'nombre' => 'Carolina Méndez',
            'telefono' => '2494001002',
            'direccion' => 'San Martín 620, Tandil',
            'abogado_id' => $valentina->id,
        ]
    );

    return response()->json([
        'estudio' => $estudio->only('id', 'nombre', 'slug'),
        'luciano' => $luciano->only('id', 'name', 'email', 'estudio_id'),
        'valentina' => $valentina->only('id', 'name', 'email', 'estudio_id'),
        'ricardo' => $ricardo->only('id', 'nombre', 'abogado_id'),
        'carolina' => $carolina->only('id', 'nombre', 'abogado_id'),
    ]);
});
